...
This is **Anthea's 2025 iMessage Wrapped** — a personalized year-in-review visualization of iMessage data, similar to Spotify Wrapped but for text messages. It displays message statistics, top contacts, emoji usage, tapback reactions, memorable messages, connection vibes, and even a personality evaluation based on messaging patterns.

## Key Features

### Three View Modes
- **Vibes** — Charts and stats: message volume, top contacts, emoji usage, tapbacks, connection vibes, plans
- **Messages** — Curated memorable texts: first/last messages, nice memories, messages by reaction type
- **Personality** — Graham Duncan-style talent evaluation based on message patterns

### Instagram Story Export
Every card can be exported as a 1080×1920 Instagram Story image:

1. **Individual Export**: Hover over any card to reveal a "Story" button that exports just that card
2. **Bulk Export**: Click "Share Stories" button in the header to open a modal with:
   - "Download All" button to export all cards at once
   - Individual export buttons organized by category (Vibes, Messages, Personality)

The export system uses `html2canvas-pro` and renders dedicated story-optimized versions of each card that:
- Use larger fonts and spacing for the vertical format
- Truncate long text appropriately
- Re-render charts at story dimensions
- Include consistent branding header/footer

### Components

- `src/pages/Wrapped2025.tsx` — Main page with all card layouts
- `src/components/story-export.tsx` — Export functionality (ExportableCard wrapper, ShareModal)
- `src/components/story-cards.tsx` — Story-optimized card renderers for each card type
- `src/lib/data/*.json` — Pre-processed iMessage data files

### Data Files
The visualization reads from JSON files in `src/lib/data/`:
